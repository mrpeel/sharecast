    ...
    def compute_lr(self):
        ...
        if self.clr_iterations > 2 * self.mid_cycle_id:
            ...
            new_lr = self.initial_lr * \
                (1. + (current_percentage * (1. - 100.) / 100.)) * self.scale

        elif self.clr_iterations > self.mid_cycle_id:
            current_percentage = 1. - \
                (self.clr_iterations - self.mid_cycle_id) / self.mid_cycle_id
            new_lr = self.initial_lr * \
                (1. + current_percentage * (self.scale * 100 - 1.)) * self.scale
...
class LRFinder(Callback):
    ...
    def __init__(self, num_samples, batch_size,
        ...
        if lr_scale == 'exp':
            self.lr_multiplier_ = (
                maximum_lr / float(minimum_lr)) ** (1. / float(self.num_batches_))
        else:
    ...
    def on_batch_begin(self, batch, logs=None):
        ...
        print('Batch begin', self.current_batch_,
              'of', len(self.lr_multiplier_))

    def on_batch_end(self, batch, logs=None):
        if self.current_epoch_ > 1:
            return

        # self.current_batch_ = min(
        #     self.current_batch_, len(self.lr_multiplier_))
        ...
        running_loss = self.loss_smoothing_beta * \
            loss + (1. - self.loss_smoothing_beta) * loss
        ...
        if self.lr_scale == 'exp':
            ...
            current_lr = self.lr_multiplier_[self.current_batch_ - 2]
