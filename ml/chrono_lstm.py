...
class ChronoLSTMCell(Layer):
    ...
    def __init__(self, units, max_timesteps,
        ...
        self.units = units
        self.max_timesteps = max_timesteps if max_timesteps is not None else int(
            10000)
        ...
        if self.use_bias:
            if self.use_chrono_initialization:
                def bias_initializer(_, *args, **kwargs):
                    f_init = ChronoInitializer(self.max_timesteps)(
    ...
    def call(self, inputs, states, training=None):
        if 0 < self.dropout < 1 and self._dropout_mask is None:
            self._dropout_mask = _generate_dropout_mask(
                K.ones_like(inputs, K.shape(inputs)[-1]),
                self.dropout,
                training=training,
                count=4)
